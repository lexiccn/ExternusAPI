...
public interface RejectionPolicy {
     ...
     */
    boolean handle(@NotNull ActionBar toRender, @NotNull ActionBar currentlyRendered, @NotNull Queue<ActionBar> queued);
     ...
     */
    static RejectionPolicy OVERWRITE() {
        return (toRender, currentlyRendered, queued) -> true;
    }
     ...
     */
    static RejectionPolicy SILENT_REJECT() {
        return (toRender, currentlyRendered, queued) -> false;
    }
     ...
     */
    static RejectionPolicy FAIL() {
        return (toRender, currentlyRendered, queued) -> {
     ...
     *
     * @return The queue rejection policy
     */
    static RejectionPolicy QUEUE() {
        return (toRender, currentlyRendered, queued) -> {
            queued.add(toRender);
            return false;
        };
