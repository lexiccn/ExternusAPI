...
import me.deltaorion.bukkit.display.actionbar.*;
import me.deltaorion.bukkit.display.actionbar.renderer.FailRenderer;
import me.deltaorion.bukkit.display.actionbar.renderer.TestRenderer;
...
public class ActionBarTest implements MinecraftTest {
    ...
    private void runFunctionalityTest() {
        ...
        Assert.assertEquals(actionBar, player.getActionBarManager().getCurrentActionBar());
        assertNotNull(actionBarRunning.get());

        Assert.assertEquals(actionBar, actionBarRunning.get().getActionBar());
    ...
    private void runRejection() {
        ...
        Assert.assertEquals(gamer2, player.getActionBarManager().getCurrentActionBar());
        player.getActionBarManager().removeActionBar();
        try {
            assertTrue(actionBarRunning.get().getFinishLatch().await(1000, TimeUnit.MILLISECONDS));
        } catch (InterruptedException e) {
